...
// ── AI AUTOMATION (Gemini API Free Tier Integration) ──
async function generateAiPost() {
  ...
  setStatus('Step 1: Writing text with Gemini 1.5 Flash...');

  try {
    // 1. Ask Gemini 1.5 Flash for Text and an Image Prompt (Free Tier)
    ...
    if (!textRes.ok) {
      ...
      throw new Error(err.error?.message || "Text API Key invalid or rejected.");
    }
    ...
    setStatus('Step 2: Generating Image with Gemini API...');

    // 3. Generate Image using Imagen 3 API (Google AI Studio Free Tier)
    const imageUrlEndpoint = `https://generativelanguage.googleapis.com/v1beta/models/imagen-3.0-generate-001:predict?key=${apiKey}`;
    
    // We append specific style instructions to ensure the image fits the quote graphic
    const finalImagePrompt = content.imagePrompt + ", peaceful, spiritual, cinematic lighting, no text, empty space in center";

    const imageRes = await fetch(imageUrlEndpoint, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({
        ...
        parameters: {
          sampleCount: 1,
          aspectRatio: "1:1", // Forces the image to be a square, perfect for your 1080x1080 canvas
          outputOptions: {
            mimeType: "image/jpeg"
          }
        }
    ...
    if (!imageRes.ok) {
      ...
      throw new Error(errData.error?.message || "Image generation failed. Ensure your API key has Imagen access.");
    }

    // Google API returns images as Base64 strings, not URLs
    ...
    const img = new Image();
    img.crossOrigin = "Anonymous"; // Still good practice, though not strictly required for base64
    img.onload = () => {
      bgImage = img;
      renderCanvas();
      btn.disabled = false;
    ...
    // Set the source to the Base64 string generated by the API
    img.src = imageSrc;
