...
// ── AI AUTOMATION (Dynamic Model Discovery + Pollinations Image) ──
async function generateAiPost() {
  ...
  btn.disabled = true;
  btn.textContent = '⏳ Finding valid Gemini model...';
  setStatus('Step 1: Checking API key permissions...');

  try {
    // 1. DYNAMICALLY DISCOVER A VALID MODEL
    const listUrl = `https://generativelanguage.googleapis.com/v1beta/models?key=${apiKey}`;
    const listRes = await fetch(listUrl);
    if (!listRes.ok) throw new Error("Could not fetch model list. Check if API key is valid.");
    
    const listData = await listRes.json();
    
    // Find the first model that has "flash" in the name AND supports text generation
    let targetModel = listData.models.find(m => 
      m.name.includes('flash') && 
      m.supportedGenerationMethods.includes('generateContent')
    );
    
    // Fallback: If no flash model is found, just grab the first model that works
    if (!targetModel) {
      targetModel = listData.models.find(m => m.supportedGenerationMethods.includes('generateContent'));
    }
    ...
    const modelEndpointName = targetModel.name; // e.g., "models/gemini-1.5-flash"
    console.log("✅ Successfully dynamically targeted:", modelEndpointName);

    // 2. ASK GEMINI FOR TEXT USING THE DISCOVERED MODEL
    btn.textContent = '⏳ Gemini is thinking...';
    setStatus(`Step 2: Writing text with ${modelEndpointName.replace('models/', '')}...`);
    
    const textUrl = `https://generativelanguage.googleapis.com/v1beta/${modelEndpointName}:generateContent?key=${apiKey}`;
    ...
    setStatus('Step 3: Generating Background Image...');
    ...
    setStatus('Step 4: Loading to Canvas...');
