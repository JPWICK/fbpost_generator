...
async function generateAiPost() {
  ...
  try {
    ...
    // 3. Generate Image using Gemini 2.5 Flash Image API (Google AI Studio Free Tier)
    const imageUrlEndpoint = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-image:generateContent?key=${apiKey}`;
    ...
    if (!imageRes.ok) {
      ...
      throw new Error(errData.error?.message || "Image generation failed. Ensure your model ID is correct.");
    }

    // Google AI Studio API returns images as Base64 strings inside the 'inlineData' object
    const imageData = await imageRes.json();
    const base64Image = imageData.candidates[0].content.parts[0].inlineData.data;
    const mimeType = imageData.candidates[0].content.parts[0].inlineData.mimeType || "image/jpeg";
